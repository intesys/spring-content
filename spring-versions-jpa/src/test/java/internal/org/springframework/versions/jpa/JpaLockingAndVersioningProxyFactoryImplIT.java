...
@DisplayName("JpaLockingAndVersioningProxyFactoryImpl")
public class JpaLockingAndVersioningProxyFactoryImplIT {
    ...
    class WithContext {
        ...
        class Apply {
            ...
            @Nested
            @DisplayName("given no existng advise")
            class NoExistingAdvice {
                ...
                @Test
                @DisplayName("should apply the txn advice")
                void shouldApplyTxnAdvice() {
            ...
            @Nested
            @DisplayName("given an existng txn advise")
            class ExistingTxnAdvice {
                ...
                @Test
                @DisplayName("should not apply the advice again")
                void shouldNotApplyAgain() {
                    Advisor[] advices = proxyFactory.getAdvisors();
                    assertThat(advices.length, is(3));
                    assertThat(advices[0].getAdvice(), is(instanceOf(TransactionInterceptor.class)));
                    assertThat(advices[1].getAdvice(), is(instanceOf(OptimisticLockingInterceptor.class)));
